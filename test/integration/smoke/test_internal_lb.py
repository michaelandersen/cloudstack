...
class TestInternalLb(cloudstackTestCase):
    ...
    def create_and_enable_network_serviceoffering(self, services):
        ...
        try:
            ...
            self.fail("Failed to create and enable network offering: %s because of %s"%(offering_name, e))
    ...
    def deployvm_in_network(self, vpc, networkid):
        ...
        try:
            ...
            vm = VirtualMachine.create(self.apiclient, services=self.services["virtual_machine"],
                                       ...
                                       hypervisor=self.services["template_kvm"]["hypervisor"]
                                       )
    ...
    def create_natrule(self, vpc, vm, public_port, private_port, public_ip, network, services=None):
        ...
        self.assertIsNotNone(nwacl_nat, "Failed to create acl rule for %s" %vm.id)
    ...
    def run_ssh_test_accross_hosts(self, clienthost, lb_address, max_requests=30):
        ...
        try:
            for x in range(0, max_requests):
                ...
                #self.debug( "SSH into VM public address: %s and port: %s" %(.public_ip, vm.public_port))
    ...
    def evaluate_http_responses(self, responses, algorithm):
        ...
        if algorithm == 'roundrobin':
            ...
            if len(set(dataset)) == 1:
                # all values in dataset are equal, perfect result distribution woohoo!
                ...
                for value in dataset:
                    # determine how much value difference is there from the mean
                    difference = abs(value - mean)
                    # difference between response count of a host and the mean should be less than the standard deviation
    ...
    def test_01_internallb_roundrobin_1VR_2VM_TCP_port22(self):
        ...
        self.logger.debug("Starting test_01_internallb_roundrobin_2VM_port80")
        ...
        self.create_natrule(vpc, client_vm, public_ssh_start_port, private_ssh_port, guestnet_public_ip, network_guestnet)

        # Create nat rule to access app vms directly and start a http daemon on the vm
        public_port = public_ssh_start_port + 1
        for vm in app_vms:
            self.create_natrule(vpc, vm, public_port, private_ssh_port, intlb_public_ip, network_internal_lb)
            public_port += 1
    ...
    def test02_internallb_haproxy_stats_on_all_interfaces(self):
        ...
        # Verify access to and the contents of the admin stats page on the private address via a vm in the internal lb tier
